...
dialog.setStyles({left:xleft,top:xtop,position:'absolute'});};setSize();window.addEvent('resize',setSize);},'hide':function(){try{this.dialog.destroy();window.removeEvent('resize',this.setSize);}catch(e){}},'load':function(options){var jqTarget=jQuery(options.target);options=options||{};options=Object.merge({method:'post',onRequest:function(){miniCart.show(options.target);},onSuccess:function(re){if(re){this.title.innerHTML=memo2;this.content.innerHTML=re;var reTest=/加入购物车成功/gi;var result=reTest.test(re);if(result){if(typeof(sasaCurrentPage)!=="undefined"&&sasaCurrentPage=='productpage'){var pyId='',pyCount='',pyMoney='',pyPrice='';pyPrice=jQuery('.price .action-price').text().replace("￥","");pyId=document.getElementsByName('goods[product_id]')[0].value;if(jqTarget.attr('target')=='_dialog_minicart'){pyCount=1;}
else{pyCount=jQuery('.action-quantity-input').val();}
pyMoney=pyPrice*pyCount;py('event','viewCart',{'money':pyMoney,'items':[{'id':pyId,'count':pyCount,'price':pyPrice}]}).track('pr.GC2.hZ0pDAfyIvG5iP0Xn3oA1X');}}}
